...
export default class webGL {
  ...
  updateBgMouse() {
    this.bgTarget.set((this.mouse.current.x + 1) * 0.5,  (this.mouse.current.y + 1) * 0.5);
    this.material.uniforms.uMouse.value.lerp(this.bgTarget, 0.2);
    // this.material.uniforms.uMouse.value = lerp(this.bgTarget, this.mouse.target, 0.2);
    this.material.uniforms.uTime.value += 0.005;
  }

  updateLenseMouse() {
    this.lensTarget.set(this.mouse.current.x, this.mouse.current.y, 0.01);
    this.lenseMesh.position.lerp( this.lensTarget, 0.1);
  }

  updateTextMouse() {
    this.textPlaneTarget.set(this.mouse.current.x, this.mouse.current.y);
  ...
  update() {
    ...
    // this.bgTarget.set((this.mouse.current.x + 1) * 0.5,  (this.mouse.current.y + 1) * 0.5);
    // this.material.uniforms.uMouse.value = lerp(
    //   this.mouse.current,
    //   this.mouse.target,
    //   10.00002
    // );
